  ...
  setup() {
    ...
    onMounted(async () => {
      if (state.types.length <= 0) {
        const res = await api.material.getKinds({ type: 4 })
        const types = res.data
        state.types = types
        for (const iterator of types) {
          const { data } = await api.material.getImagesList({ cate: iterator.id, pageSize: 2 })
          state.showList.push(data.data)
        }
      }
    })
    ...
    const getDataList = async () => {
      ...
      let {
        data: { data: list = [] },
        total,
      } = await api.material.getImagesList({ cate: state.currentCategory.id, page: state.page, pageSize: 30 })
