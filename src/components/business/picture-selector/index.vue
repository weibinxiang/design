...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const loadPic = (init?: boolean) => {
      ...
      loading = true
      picPage += 1
      api.material.getImagesList({ page: picPage }).then(({ data }: any) => {
        data.data.length <= 0 ? (state.isPicsDone = true) : (state.recommendImgList = state.recommendImgList.concat(data.data))
        setTimeout(() => {
          loading = false
        }, 100)
      })
